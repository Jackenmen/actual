...
async function updateAccountIban(id, iban) {
  await db.runQuery('UPDATE accounts SET iban = ? WHERE id = ?', [
    iban,
    id,
  ]);
}
...
export async function syncNordigenAccount(userId, userKey, id, acctId, bankId) {
  ...
  if (latestTransaction) {
    ...
    let { iban, transactions, accountBalance } = await downloadNordigenTransactions(
      userId,
      userKey,
      acctId,
      bankId,
    ...
    const { iban, transactions, startingBalance } =
